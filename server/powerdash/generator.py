...
class generator:
    def __init__(self):
        ...
        self.config_database = create_engine('sqlite:///config.db')
    def config_generation(self,file_name, csv):
        ...
        i = 0
        j = 1
        for df in pd.read_csv(file_name, chunksize=chunksize, iterator=True, names=['BldId', 'MeterId', 'Description', 'Units', 'Resource', 'BuildingName','SqrFeet','BuildDate','Lat','Long','Campus','Organization','LocationType','SteamID', 'ChilledWater','HotWaterID','ActivationDate']):
    ...
    def initialize_generator(self,config, hourly_one, hourly_two, daily):
        self.config_generation(config, self.config_database)
